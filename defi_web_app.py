#!/usr/bin/env python3
"""
PRODUCTION DEFI SECURITY WEB APPLICATION
Complete system with real blockchain data, web GUI, and real-time monitoring
...
# Try to import Web3 (optional for demo)
try:
    from web3 import Web3
    WEB3_AVAILABLE = True
except:
    WEB3_AVAILABLE = False
    print("⚠️  Web3 not available. Using simulated blockchain data.")
...
def monitor_transactions():
    ...
    while state.monitoring_active:
        # Simulate new transactions (90% normal, 10% exploits for demo)
        ...
        # Sleep for demo (simulate real-time)
        time.sleep(1)
...
<body>
    <div class="header">
        <h1>🛡️ DeFi Security Dashboard</h1>
        ...
        <div class="table-card">
            ...
            <table>
                ...
                <tbody id="transactions-table">
                    <tr><td colspan="6" style="text-align:center;">Waiting for transactions...</td></tr>
                </tbody>
            </table>
        </div>
    ...
    <div class="footer">
        Multi-Chain ML Security Framework | Built with Python, Flask & ML | 2025
    </div>
                ...
                if (alertsData.alerts.length > 0) {
                    alertsContainer.innerHTML = alertsData.alerts.map(alert => `
                        <div class="alert-card ${alert.type === 'Critical' ? 'alert-critical' : ''}">
                            <strong>${alert.type} Alert</strong> - ${alert.chain}<br>
                            Hash: ${alert.hash} | Risk: ${(alert.risk_score * 100).toFixed(0)}% | 
                            Value: $${alert.value_usd.toFixed(2)}
...
def main():
    ...
    print("\n" + "="*70)
    print("🚀 PRODUCTION DEFI SECURITY WEB APPLICATION")
    print("="*70)
    print("\n📊 Initializing system...")
    ...
    # Start Flask app on port 8080 (to avoid macOS AirPlay conflict)
